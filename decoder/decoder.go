...
type Decoder interface {
	...
	StartDatabase(n int, offset int) error
	// AUX field
	Aux(key, value []byte) error
	// ModuleAux()
	ModuleAux(modName []byte) error
	// ResizeDB hint
	...
	EndDatabase(n int, offset int) error
	// EndRDB is called when parsing of the RDB file is complete.
	EndRDB(offset int) error
}
...
func (d *decode) decode() error {
	...
	for {
		...
		case rdbFlagModuleAux:
			...
			if err := d.event.ModuleAux(modName); err != nil {
				return err
			}
		...
		case rdbFlagSelectDB:
			if !firstDB {
				if err := d.event.EndDatabase(int(db), d.offset); err != nil {
					return err
				}
			}
			...
			if err := d.event.StartDatabase(int(db), d.offset); err != nil {
				return err
			}
			firstDB = false
		case rdbFlagEOF:
			...
			if err := d.event.EndRDB(d.offset); err != nil {
				return err
			}
			return nil
		default:
...
func readZiplistLength(buf *sliceBuffer) (int64, error) {
	if _, err := buf.Seek(8, 0); err != nil { // skip the zlbytes and zltail
		return 0, err
	}
